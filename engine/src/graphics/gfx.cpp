...
void Renderer::draw(const SpriteEntry& entry, const vec2f& pos, float rad, float scale) const {
	...
	float hw = entry.hw * scale;
	float hh = entry.hh * scale;
...
void Renderer::drawCollider(const vec2f pos, float px) const {
    SDL_Renderer* renderer = static_cast<SDL_Renderer*>(native);

	const SpriteEntry& entry = spriteTable[ static_cast<size_t>(SpriteID::collider) ][0];
	draw(entry, pos, 0, px);
}
